...
try:
    ...
    z_coefficient = np.array([[325, 225, 210, 115, 75, 50]])
    z_coefficient = np.repeat(z_coefficient, 3, axis=0)
    z = MODEL.addVars(index, vtype=GRB.CONTINUOUS, name="z")
    MODEL.update()
    MODEL.addConstrs((gp.quicksum(x[i, j] for i in index) <= 1 for j in xj), "x")
    MODEL.addConstrs((z[i] == gp.quicksum(z_coefficient[i][j] * x[i, j] for j in xj) for i in index))
